...
class MuralComando:    
    def listar(self, cliente, grupo, mensagem):
        my = MysqlHelp();
        js = mensagem.toJson();
        sql = "select * from mural where id_nivel in ( select id_nivel from nivel_cliente where id_cliente = %s ) or id_destinatario=%s ";
        lista = my.datatable(sql, [ cliente.id, cliente.id ]);
        return {"lista" : lista };
