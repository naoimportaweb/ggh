...
class Operacao():
    ...
    def toJson(self):
        ...
        return {"id" : js["id"], "id_nivel" : js["id_nivel"], "sigla" : js["sigla"], "nome" : js["nome"],"id_operacao_status" : js["id_operacao_status"],"nome_operacao_status" : self.operacao_status["nome"],"data_inicio" : js["data_inicio"],"data_fim" : js["data_fim"],"missao" : js["missao"],"foco" :  js["foco"], "atividades" : self.atividades, "niveis" : self.niveis };
    @staticmethod
    def listar(id_nivel):
        my = MysqlHelp();
        sql = "select * from nivel where id=%s";
        nivel = my.datatable( sql, [ id_nivel ])[0];
    ...
    def listar_json(id_nivel):
        buffers_json = [];
        buffers_objeto = Operacao.listar(id_nivel);
