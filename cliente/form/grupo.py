...
class PainelChat(QtWidgets.QWidget):
    def __init__( self, xmpp_var):
        ...
        #self.d_scroll_area_widget.setStyleSheet("QWidget{background-color:white;color:black}");
        ...
        #self.form_layout_chat.addLayout(    self.area_adicionar_mensagem   );
        ...
        self.thread_atualizar_chat = threading.Thread(target = self.atualizar_chat, args=());
        self.thread_atualizar_chat.start();
        self.setLayout( self.form_layout );
    
    def btn_atualizar_click(self):
        index = self.lw.currentRow();
        if index != -1:
            self.mensagens( self.xmpp_var.grupo.niveis[index]["id"]);
        else:
    ...
    def mensagens(self, nivel):
        ...
        for arquivo in arquivos:
            ...
            self.area_adicionar_mensagem.addWidget( item );
    ...
    def btn_envio_click(self):
        ...
        if self.lw.currentRow() < 0:
            ...
            return;
        if self.txt_mensagem.toPlainText().strip() != "":
            self.xmpp_var.adicionar_mensagem( "comandos.mensagem" ,"Mensagem", "lista_clientes_niveis", {"nivel" : self.xmpp_var.grupo.niveis[self.lw.currentRow()]["id"]} );
    ...
    def evento_mensagem(self, de, texto, message, conteudo_js):
        if conteudo_js["comando"] == "GrupoCadastro":
            ...
            if self.lw.currentRow() < 0:
                ...
                self.mensagens( self.xmpp_var.grupo.niveis[0]["id"] );
        if conteudo_js["comando"] == "Mensagem" and conteudo_js["funcao"] == "lista_clientes_niveis":
            ...
            self.xmpp_var.adicionar_mensagem( "comandos.mensagem" ,"Mensagem", "listar", {"id_nivel" : self.xmpp_var.grupo.niveis[self.lw.currentRow()]["id"] } );
        ...
        if conteudo_js["comando"] == "Mensagem" and conteudo_js["funcao"] == "listar":
            ...
            for mensagem in conteudo_js["retorno"]:
                ...
                self.mensagens(mensagem["id_nivel"]);
